...
class BiLSTMModel(nn.Module):
    def __init__(self, vocab_size, num_hiddens, num_classes=3):
        ...
        self.bilstm = nn.LSTM(num_hiddens, num_hiddens, batch_first=True, bidirectional=True)
        self.fc = nn.Linear(num_hiddens * 2, num_classes)
...
for otimizador in ['Adam', 'SGD', 'RMSprop']:
    for lr in [0.0001, 0.001, 0.01]:
      for num_hiddens in [16, 32, 64]:
        ...
        bilstm_model = BiLSTMModel(vocab_size=num_inputs, num_hiddens=num_hiddens)

        criterion = nn.CrossEntropyLoss()
        if otimizador == 'Adam':
            optimizer = torch.optim.Adam(bilstm_model.parameters(), lr=lr)
        elif otimizador == 'SGD':
            optimizer = torch.optim.SGD(bilstm_model.parameters(), lr=lr, momentum=0.9)
        elif otimizador == 'RMSprop':
            optimizer = torch.optim.RMSprop(bilstm_model.parameters(), lr=lr, alpha=0.9)
        num_epochs = 10
        num_epochs = 10
